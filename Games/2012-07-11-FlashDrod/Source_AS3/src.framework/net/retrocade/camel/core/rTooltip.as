package net.retrocade.camel.core{
    ...
    public class rTooltip extends Sprite{
        ...
        retrocamel_int function update():void{
            ...
            x = rDisplay._stage.mouseX - _txt.width / 2 - (_paddingLeft + _paddingRight) / 2 | 0;
            y = rDisplay._stage.mouseY - _txt.height    -  _paddingTop  - _paddingBottom - 8 | 0;

            if (x < 0) x = 0;
            if (x + width > rCore._settings.SIZE_GAME_WIDTH) x = rCore._settings.SIZE_GAME_WIDTH - width;

            if (y < 0) y = 0;
            if (y + height > rCore._settings.SIZE_GAME_HEIGHT) y = rCore._settings.SIZE_GAME_HEIGHT - height;
        }
