package net.retrocade.camel.core{
    ...
    public class rSound{
        ...
        public static function playSound(sound:*, soundName:String = null, soundVolume:Number = NaN, soundPan:Number = 0):SoundChannel{
            ...
            var soundTransform:SoundTransform = new SoundTransform(_soundVolume.volume);

            if (!isNaN(soundVolume)) {
                soundTransform = new SoundTransform(soundVolume, soundPan);
            }

            if (soundTransform.volume < 0.01) {
                return null;
            }

            soundTransform.volume = Math.round(soundTransform.volume * 100) / 100;

            var sc:SoundChannel = Sound(sound).play(25, 0, soundTransform);
