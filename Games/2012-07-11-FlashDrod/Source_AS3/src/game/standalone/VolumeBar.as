package game.standalone{
    ...
    public class VolumeBar extends rObjectDisplay{
        ...
        override public function update():void{
            if (_dragStarted){
                var valueTemp:Number = UNumber.limit((_display.mouseX + _width / _bars) / _width, 1, 0);
                valueTemp = (valueTemp * _bars | 0) / _bars;
                
                if (valueTemp != _value)
                    value = valueTemp;
                    
                if (!rInput.isMouseDown)
                    _dragStarted = false;
            }
        }
