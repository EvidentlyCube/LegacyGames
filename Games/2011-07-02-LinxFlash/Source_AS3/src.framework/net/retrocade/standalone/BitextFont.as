package net.retrocade.standalone {
    ...
    public class BitextFont { 
         ...
         */
        private static var _default:BitextFont;
        
        /**
         * The default font used by the Bitext (the first set font). Can't be set to null
         */
        public static function get defaultFont():BitextFont{
            return _default;
        }
         ...
         * @private 
         */
        public static function set defaultFont(value:BitextFont):void{
            if (value == null)
         ...
         */
        public function BitextFont(bitmapClass:Class, charWidth:uint, charHeight:uint, monospace:Boolean = false, spaceWidth:uint = 1, characterList:String = null) {
            if (!characterList)
                characterList = DEFAULT_CHARACTER_LIST;
            
            _bitmapdata = rGfx.getBD(bitmapClass);
         ...
         */
        retrocamel_int function drawText(bitext:Bitext):BitmapData {
            if (bitext.text == "")
                return null;
            ...
            for (i = 0; i < l; i++) {
                switch(bitext.align) {
                    case Bitext.ALIGN_LEFT:
                        drawLine(bitmap, text[i], 0, i * (_bitmapCharHeight + bitext.lineSpace), bitext);
                        break;
                        
                    case Bitext.ALIGN_MIDDLE:
                        drawLine(bitmap, text[i], (calcWidth - linesWidth[i]) / 2, i * (_bitmapCharHeight + bitext.lineSpace), bitext);
                        break;
                        
                    case Bitext.ALIGN_RIGHT:
                        drawLine(bitmap, text[i], calcWidth - linesWidth[i], i * (_bitmapCharHeight + bitext.lineSpace), bitext);
                        break;
                }
            }
         ...
         */
        retrocamel_int function drawLine(bitmap:BitmapData, text:String, x:uint, y:uint, bitext:Bitext):void {
            var i    :int  = 0;
            var l    :int  = text.length;
            var c    :uint = 0;
