package net.retrocade.sfxr {
	...
	public class SfxrSynth {
		...
		public function get params():SfxrParams {
			return _params;
		}                            // Sound instance used to play the sound
		
		public function set params(value:SfxrParams):void {
			_params = value;
			_params.paramsDirty = true;
		}                    // SoundChannel instance of playing Sound
		private var _sound:Sound;                        // If the current sound playing or caching is a mutation
		private var _channel:SoundChannel;                    // Cached wave data from a cacheSound() call
		private var _mutation:Boolean;                    // If the synth is caching a normal sound
		private var _cachedWave:ByteArray;                    // Current caching ID
		private var _cachingNormal:Boolean;                // Current caching wave data for mutation
		private var _cachingMutation:int;    // Cached mutated wave data
		private var _cachedMutation:ByteArray;                // Number of cached mutations
		private var _cachedMutations:Vector.<ByteArray>;            // Amount to mutate during cache
		private var _cachedMutationsNum:uint;                    // If the synth is currently caching asynchronously
		private var _cachedMutationAmount:Number;                // Maximum time allowed per frame to cache sound asynchronously
		private var _cachingAsync:Boolean;                // Function to call when finished caching asynchronously
		private var _cacheTimePerFrame:uint;                        // Shape used for enterFrame event
		private var _cachedCallback:Function;                    // Full wave, read out in chuncks by the onSampleData method
		private var _cacheTicker:Shape;                        // Current position in the waveData
		private var _waveData:ByteArray;                    // Number of bytes in the waveData
		private var _waveDataPos:uint;                    // Number of bytes to write to the soundcard
