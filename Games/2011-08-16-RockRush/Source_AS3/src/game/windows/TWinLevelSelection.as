package game.windows {
	...
	public class TWinLevelSelection extends RetrocamelWindowFlash {
		...
		public function generateLevelButtons():void {
			...
			const LEVELS_PER_ROW:uint = LevelList.levelCount(S().currentVersion) / 2 - 1 | 0;
			...
			for (var i:int = 0; i < LevelList.levelCount(S().currentVersion); i++) {
				...
				button.addEventListener(MouseEvent.ROLL_OVER, onButtonRolledOver, false, 0, true);
				_buttonContainer.addChild(button);

				button.x = lastX;
				button.y = lastY;

				if (i == (LevelList.levelCount(S().currentVersion) / 2 - 1 | 0)) {
					lastX = START_X;
