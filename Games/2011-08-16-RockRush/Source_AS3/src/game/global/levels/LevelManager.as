package game.global.levels {
	...
	public class LevelManager {
		...
		public static function levelCompleted():void {
			...
			var wasGameCompleted:Boolean = Score.gameCompleted();

			Score.setCompleted(currentRoom, true);

			if (!wasGameCompleted && Score.gameCompleted()) {
				TStateWin.set();
				return;

			} else if (wasGameCompleted) {
		...
		public static function loadLevelFromScratch(id:uint):void {
			var level:XML = LevelList.getLevelXml(S().currentVersion, id);
			var item:XML;
