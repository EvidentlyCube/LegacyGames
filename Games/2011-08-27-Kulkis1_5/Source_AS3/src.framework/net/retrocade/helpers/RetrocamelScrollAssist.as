        ...
        public function update():void {
            if (!_isScrollLocked) {
                if (_horizontalScrollEnabled && _x != _toX) {
                    _x = UtilsNumber.limit(UtilsNumber.approach(_x, _toX, 0.1), cornerRightWithoutWidth(), _cornerLeft);
                }

                if (_verticalScrollEnabled && _y != _toY) {
                    _y = UtilsNumber.limit(UtilsNumber.approach(_y, _toY, 0.1), cornerBottomWithoutHeight(), _cornerTop);
                }
            }
        }
