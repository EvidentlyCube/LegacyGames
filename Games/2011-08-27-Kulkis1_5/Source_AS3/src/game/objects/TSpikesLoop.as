package game.objects {
	 ...
	 * @author
	 */
	public class TSpikesLoop extends TGameObject {
		...
		public function TSpikesLoop(x:Number, y:Number, nodes:XML, speed:Number, turnWait:uint) {
			_x = x;
			_y = y;
		...
		override public function update():void {
			...
			if (_waiter) {
				...
				if (Math.abs(_nodes[_nodeNow].y - _y) <= 1 && Math.abs(_nodes[_nodeNow].x - _x) <= 1) {
					...
					_nodeNow = (_nodeNow + 1) % _nodes.length;
			...
			if (UtilsObjects.distanceSquaredFromCenter(this, player) < 10 * 10) {
				player.kill();
			}
		}
	}
