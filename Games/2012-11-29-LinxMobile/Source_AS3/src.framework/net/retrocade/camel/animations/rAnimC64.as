    ...
    public class rAnimC64 extends rObjectDisplay{
        ...
        public function rAnimC64(padColor:uint, padTop:Number = NaN, padLeft:Number = NaN, padRight:Number = NaN, padBottom:Number = NaN, scale:Number = 1) {
            ...
            if (!isNaN(padTop) && !isNaN(padLeft) && !isNaN(padRight) && !isNaN(padBottom)){
                ...
                _padding.graphics.drawRect(padLeft, padTop, rCore.settings.gameWidth - padLeft - padRight, 
                                                            rCore.settings.gameHeight - padTop - padBottom);
        ...
        override public function update():void {
            ...
            while (hei < rCore.settings.gameHeight) {
                for each(o in _all) {
                ...
                g.drawRect(0, hei, rCore.settings.gameWidth, thick);
                hei += thick;
            }
        }
        
        public function addColor(color:uint, minThick:Number, maxThick:Number, probability:Number):void {
            _all.push({
                color:       color, 
                min:         minThick, 
                max:         maxThick, 
