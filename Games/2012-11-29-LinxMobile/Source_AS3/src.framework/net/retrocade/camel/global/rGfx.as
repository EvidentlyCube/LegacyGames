package net.retrocade.camel.global{
    ...
    public class rGfx{
		...
		private static function bitmapLoaded(event:Event):void{
            ...
            var retroBitmap:rBitmap = new rBitmap(bitmap.bitmapData);
            
			_graphics[gfxClass] = retroBitmap;
		}
         ...
         */
        public static function getB(gfx:Class):rBitmap{
            if (!_graphics[gfx]){
                var bitmap:Bitmap = new gfx;
                
                _graphics[gfx] = new rBitmap(bitmap.bitmapData);
            }

            return new rBitmap(rBitmap(_graphics[gfx]).bitmapData);
        }
         ...
         * @return Bitmap od the specified class
         */
        public static function getBExt(gfx:Class, x:uint, y:uint, width:uint, height:uint):rBitmap{
            var code:String = Object(gfx).toString() + ":" + x + ":" + y + ":" + width + ":" + height;
            
            if (_graphics[code])
                return new rBitmap(rBitmap(_graphics[code]).bitmapData);
