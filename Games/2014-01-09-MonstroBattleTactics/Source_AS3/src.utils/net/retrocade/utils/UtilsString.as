...
package net.retrocade.utils {
    ...
    */
    public class UtilsString {
        ...
        * @param versionLeft
        * @param versionRight
        * @return 1 if right version is higher, -1 if right version is smaller, or 0 when they are equal
        */
       public static function compareVersions(versionLeft:String, versionRight:String):int{
           if (!versionLeft || !versionRight){
               return 0;
           }

           var leftSections:Array = versionLeft.split(".");
           var rightSections:Array = versionRight.split(".");

           var l:int = Math.min(leftSections.length, rightSections.length)
           for (var i:int = 0; i < l; i++){
               var left:int = parseInt(leftSections[i]);
               var right:int = parseInt(rightSections[i]);

               if (left !== right){
                   return UtilsNumber.sign(right - left);
               }
           }
