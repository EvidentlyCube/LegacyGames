...
package net.retrocade.retrocamel.sound {
    ...
    public class RetrocamelSound implements IRetrocamelSound{
        ...
        private var _isRegisteredAsMusic:Boolean;
        private var _isRegisteredAsSound:Boolean;
        ...
        public function play(loop:uint = 0, volumeModifier:Number = 1, pan:Number = 0):void {
			...
			if (_isRegisteredAsMusic && RetrocamelSoundManager.suppressMusic){
				pause();
			}
        }
        ...
        public function registerAsMusic():void {
            ...
            _isRegisteredAsMusic = true;
            RetrocamelSoundManager.registerMusic(this);
			if (RetrocamelSoundManager.suppressMusic){
				_maxVolume = 0;
				if (isPlaying){
					pause();
				}
        ...
        public function registerAsSoundEffect():void {
            if (isRegistered) {
                throw new IllegalOperationError("This sound was already registered");
            }
            _isRegisteredAsSound = true;
            RetrocamelSoundManager.registerSound(this);
			if (RetrocamelSoundManager.suppressSounds){
				_maxVolume = 0;
			}
        }
        ...
        public function set volume(value:Number):void {
            _playVolume = Math.max(0, Math.min(1, value));
		...
		private function get isRegistered():Boolean{
			return _isRegisteredAsMusic || _isRegisteredAsSound;
		}
    }
}