...
package net.retrocade.retrocamel.core{
    ...
    public class RetrocamelSoundManager{
         ...
         * Prevents sound effects from being played
         */
        public static function get suppressMusic():Boolean{
            return rSfxMusicManager.suppressMusic;
        }

        /**
         * @private
         */
        public static function set suppressMusic(value:Boolean):void{
			rSfxMusicManager.suppressMusic = value;
        }
...
class rSfxMusicManager{
	...
	private static var _suppressMusic:Boolean = false;
	...
	retrocamel_int static function get suppressMusic():Boolean {
		return _suppressMusic;
	}

	retrocamel_int static function set suppressMusic(value:Boolean):void {
		_suppressMusic = value;

		var sound:RetrocamelSound;
		if (_suppressMusic){
			pauseMusic();

			for each(sound in _registeredSounds){
				sound.maxVolume = 0;
				sound.pause();
			}
		} else {
			...
			for each(sound in _registeredSounds){
				sound.maxVolume = RetrocamelSoundManager.musicVolume;
				sound.resume();
			}
		}
	}
