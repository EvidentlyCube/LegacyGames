package {
    ...
    dynamic public class Weirdtris extends MovieClip    {
        public static var kongRestarts:uint = 0;
        ...
        public function Weirdtris(){
            ...
            //if (completedNoRestart)
                //Weirdtris.kongSubmitStats("Completed with restart", 1);

            //if (completedYesRestart)
              //  Weirdtris.kongSubmitStats("Completed without restart", 1);
            ...
            CheatCodes.AddCheat('pop', function():void {
                var tiles: Array = Weirdtris.getRandomizedBlocks(1);
                for (var i = 0; i < tiles.length; i++) {
                    var tile: Block = tiles[i];
                    tile.pop(tile.color);
                }
            });
            CheatCodes.AddCheat('boom', function():void {
                var tiles: Array = Weirdtris.getRandomizedBlocks(5);
                for (var i = 0; i < tiles.length; i++) {
                    var tile: Block = tiles[i];
                    tile.pop(tile.color);
                }
            });
            CheatCodes.AddCheat('omit', function():void {
				if (Weirdtris.currBlock.length > 0) {
					Weirdtris.completed();
				}
            });
        ...
        public static function getRandomizedBlocks(number: uint): Array {
            var allTiles: Array = Weirdtris.tiles.filter(function(item:*, index:int, array:Array): Boolean {
                return !!item;
            });

            allTiles.sort(function(a:*, b:*):int {
                return Math.random() > 0.5 ? 1 : -1;
            });

            return allTiles.slice(0, number);
        }
        ...
        public function step(e:Event):void{
            if (Block.kongTotalDestroyed > 0 && GlobalUtils.stepsFromStart % 150 == 0){
                //kongSubmitStats("Total blocks destroyed", Block.kongTotalDestroyed);
                //Block.kongTotalDestroyed = 0;
            }
        ...
        public static function restart():void{
            if (levelRestart || !gameStarted){ return; }


            kongRestarts++;
            //kongSubmitStats("Total Restarts", 1);
        ...
        public static function makeBlock(x:Number, y:Number):void{
            ...
            switch (color){
                case (0): new Block(x, y, 0, new _BLOCK_BLUE(),   true); break;
                case (1): new Block(x, y, 1, new _BLOCK_GREEN(),  true); break;
                case (2): new Block(x, y, 2, new _BLOCK_ORANGE(), true); break;
                case (3): new Block(x, y, 3, new _BLOCK_RED(),    true); break;
                case (4): new Block(x, y, 4, new _BLOCK_TURQ(),   true); break;
                case (5): new Block(x, y, 5, new _BLOCK_YELLOW(), true); break;
            }
        }
    }
}
