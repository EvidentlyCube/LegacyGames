...
package com.mauft.PlatformerEngine.objects.enemies{
	...
	public final class TChopperB extends TEnemy{
		...
		public function TChopperB(setX:Number, setY:Number, _p:TPath){
			x=setX
			y=setY
		...
		override public function Update():void{
			...
			if (___timer==120){
				...
				new TFireBullet(x,y+10,Math.atan2(Engine.player.y-y,Engine.player.x-x-5)*180/Math.PI,3)
			}
			_path.update()
			x=_path.x
			y=_path.y
		...
		override public function damage(_dam:uint):void{
			...
			if (_hp<=0){
				...
				new TMessage(Engine.player.midX,Engine.player.y, "+7 Points", 0x66FF66)
				Engine.enemyPoints+=8
