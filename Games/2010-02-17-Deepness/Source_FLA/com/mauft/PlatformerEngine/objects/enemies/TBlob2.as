package com.mauft.PlatformerEngine.objects.enemies{
	...
	public class TBlob2 extends TEnemy{
		...
		public function TBlob2(setX:Number, setY:Number, _gfx:MovieClip){
			x=setX
			y=setY
			...
			switch(_sequence){
				case(0):
					...
					if (_timer>=Math.PI*9/10){
						...
						gfx.scaleY=1
						_timer=0
						_speed=Engine.player.x>x?2:-2
						_gravity=-2-Math.random()*2
						_sequence=1
					}
					break;
				case(1):
		...
		override public function damage(_dam:uint):void{
			...
			if (_hp<=0){
				...
				new TMessage(Engine.player.midX,Engine.player.y, "+2 Points", 0x66FF66)
				Engine.enemyPoints+=2
