...
package net.retrocade.retrocamel.display.global {
    ...
    public class RetrocamelTooltip extends Sprite {
        ...
        retrocamel_int function update(event:Event):void {
            ...
            if (x + _txt.width > RetrocamelCore.settings.gameWidth) {
                x = RetrocamelCore.settings.gameWidth - _txt.width;
            }

            if (y < 0) {
                y = 0;
            }
            if (y + _txt.height > RetrocamelCore.settings.gameHeight) {
                y = RetrocamelCore.settings.gameHeight - _txt.height;
            }
        }
    }
}