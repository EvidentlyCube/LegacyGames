...
package com.mauft.PlatformerEngine{
	...
	public class Engine{
		...
		public static var layerHud:CORE_HUD								//Layer holding hud
		...
		public static var scrollHorizontal:Boolean=true				//Shall we scroll horizontally?
		public static var scrollVertical:Boolean=true				//Shall we scroll vertically?
		...
		*/
		public static function Step(e:Event):void{
			if (_restartLevel){												//Are we restarting the level?...
				...
				layerHud.setTime(time)
				layerHud.setScore(score)
				layerHud.setLives(lives)
			}
		...
		*/
		public static function updateScrolling():void{
			...
			if (scrollHorizontal){
				scrollX=-player.x-player.width/2+Settings.SCREEN_WIDTH/2
				if (cappingLeft){scrollX=Math.min(scrollX,paddingLeft)}
			}
			if (scrollVertical){
				scrollY=-player.y-player.height/2+Settings.SCREEN_HEIGHT/2
				if (cappingTop){scrollY=Math.min(scrollY,paddingTop)}
				if (cappingBottom){scrollY=Math.max(scrollY,Settings.SCREEN_HEIGHT-Settings.LEVEL_HEIGHT*Settings.TILE_HEIGHT-paddingBottom)}
			}
			scrollX=Math.round(scrollX)
			scrollY=Math.round(scrollY)
			layerBackground.x=layerForeground.x=layerEnemies.x=layerObjects.x=layerTiles.x=scrollX
			layerBackground.y=layerForeground.y=layerEnemies.y=layerObjects.y=layerTiles.y=scrollY
		}
		...
		*/
		public static function StartLevel():void{
			...
			_stage.addChild(layerHud)
			trace(layerHud.parent)
			trace(layerHud.numChildren)
			trace(layerHud.x)
			trace(layerHud.y)
			scrollLocked=false
			if (player){updateScrolling()}
		...
		*/
		public static function EndLevel():void{
			...
			if (_stage.contains(layerHud)){_stage.removeChild(layerHud)}
		...
		*/ 
		public static function Clear():void{
			...
			TElevator.blockers=new Array
			TCrate._crates=new Array
			for (var i:uint=0;i<Settings.LEVEL_WIDTH;i++){
