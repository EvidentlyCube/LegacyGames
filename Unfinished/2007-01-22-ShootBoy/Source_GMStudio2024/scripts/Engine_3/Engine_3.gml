function Engine_3() {
	...
	if !keyboard_check(vk_left) && !keyboard_check(vk_right){repeat (5) {movx-=sign(movx)*acc}}
	if !keyboard_check(vk_up) && !keyboard_check(vk_down){repeat (5) {movy-=sign(movy)*acc}}
	movx=round(movx)
	movy=round(movy)
	}
	if x<5{x=5;movx=0}
