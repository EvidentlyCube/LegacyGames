...
package starling.text
{
    ...
    public class TextField extends DisplayObjectContainer
    {
                ...
                if (isVerticalAutoSize)
                {
                    ...
                    bitmapFont.fillQuadBatch(mQuadBatch,
                        mHitArea.width, mHitArea.height, mText, mFontSize, mColor, mSmoothing,
                            mHAlign, mVAlign, mAutoScale, mKerning, mColorVector, 0, 0, mCharactersToShow, mLeading);
        ...
        public function resetColoredText():void{
            mColorVector = new Vector.<uint>();
            text = "";
            mRequiresRedraw = true;
        }

        public function appendColoredText(text:String, color:uint):void{
            var textLength:uint = text.length;
            textLength -= text.match(/\n/g).length;
            while(textLength--){
                mColorVector.push(color);
            }
        ...
        override public function clone():* {
            var clone:TextField = new TextField(width, height, text);
        ...
        override protected function cloneSetProperties(clone:*, cloneChildren:Boolean):void {
            ...
            var c:TextField = clone as TextField;

            c.mFontSize = mFontSize;
            c.mColor = mColor;
            c.mText = mText;
            c.mHtmlText = mHtmlText;
            c.mFontName = mFontName;
            c.mHAlign = mHAlign;
            c.mVAlign = mVAlign;
            c.mBold = mBold;
            c.mItalic = mItalic;
            c.mUnderline = mUnderline;
            c.mAutoScale = mAutoScale;
            c.mAutoSize = mAutoSize;
            c.mKerning = mKerning;
            c.mRequiresRedraw = true;
            c.mIsRenderedText = mIsRenderedText;
            c.mTextBounds = mTextBounds;
            c.mColorVector = UtilsArray.cloneUintVector(mColorVector);
            c.mBatchable = mBatchable;
            c.mCharactersToShow = mCharactersToShow;

            c.mLeading = mLeading;

            c.mHitArea = mHitArea ? mHitArea.clone() : null;
            c.mBorder = mBorder ? mBorder.clone() : null;

            c.mImage = null;
            c.mQuadBatch = null;

            c.mSmoothing = mSmoothing;
