...
package starling.display
{
    ...
    public class DisplayObjectContainer extends DisplayObject
    {
        ...
        public function addChildWithZ(addedChild:DisplayObject):DisplayObject
        {
            for (var i:int = 0; i < mChildren.length; i++) {
                var checkedChild:DisplayObject = mChildren[i];
                if (addedChild.z < checkedChild.z){
                    if (i > 0){
                        return addChildAt(addedChild, i - 1);
                    } else {
                        return addChildAt(addedChild, 0);
                    }
                }
            }
            return addChildAt(addedChild, numChildren);
        }
        ...
        public function removeChildren(beginIndex:int=0, endIndex:int=-1, dispose:Boolean=false):void
        {
            ...
            if (mChildren.length === endIndex - beginIndex + 1){
                mChildren.length = 0;
            } else {
                mChildren.splice(beginIndex, endIndex - endIndex + 1)
            }
        }
        ...
        /** @private */
        public function getChildEventListeners(object:DisplayObject, eventType:String,
                                                 listeners:Vector.<DisplayObject>):void
        {
            if (!object.usesEvents){
                return;
            }
        ...
        override public function clone():* {
            throw new Error("Display object container cannot be cloned");
        }
        ...
        override protected function cloneSetProperties(clone:*, cloneChildren:Boolean):void {
            ...
            var c:DisplayObjectContainer = clone as DisplayObjectContainer;

            if (cloneChildren){
                for each (var object:DisplayObject in mChildren) {
                    var clonedChild:DisplayObject = object.clone();
                    c.addChild(clonedChild);
                }
            }
        }
    }
